...
json_file = open("../data/test", "w")

def writeDataJSON(year, countrycode, location, seriesname, value):
        key = ""
        ...
        if location not in information[year][countrycode]:
                information[year][countrycode]["location"] = location
...
with open("../data/interactive_visu_data.csv", "rU") as infile:
        ...
        for row in reader:
                ...
                for i in range (0, len(years)):
                        year = years[i]                 
                        value = row[4+i]
                        writeDataJSON(year, countrycode, location, seriesname, value)
